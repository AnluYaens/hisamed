...
beforeEach(() => {
  ...
  mocks.select.mockReturnValue(selectRows([{ id: PATIENT_ID, avatarStorageKey: null }]));
...
describe('updatePatientAvatar', () => {
  it('accepts a WebP avatar and stores it re-encoded as JPEG', async () => {
    const webpBytes = await sharp({
      create: { width: 8, height: 8, channels: 3, background: { r: 90, g: 10, b: 10 } },
    })
      .webp()
      .toBuffer();
    const file = new File([webpBytes], 'foto.webp', { type: 'image/webp' });
  ...
  it('accepts a Samsung-style HEIC avatar and stores it re-encoded as JPEG', async () => {
    const file = new File([HEIC_FIXTURE], 'foto.heic', { type: 'image/heic' });
  ...
  it('rejects an undecodable image with a user-friendly error', async () => {
    ...
    expect(result).toEqual({
      success: false,
      error: 'No se pudo procesar la imagen. El archivo puede estar dañado o en un formato no compatible.',
    });
  ...
  it('rejects MIME types outside the image whitelist', async () => {
    const gif = new File([Buffer.from('GIF89a')], 'anim.gif', { type: 'image/gif' });
  ...
  it('rejects content that does not match the declared type', async () => {
    const spoofed = new File([Buffer.from('plain text payload')], 'foto.heic', {
      type: 'image/heic',
    });
