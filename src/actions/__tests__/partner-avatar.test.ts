...
beforeEach(() => {
  ...
  mocks.select
    ...
    .mockReturnValueOnce(selectRows([{ id: PARTNER_ID, avatarStorageKey: null }]));
...
describe('updatePartnerAvatar', () => {
  it('accepts a WebP avatar and stores it re-encoded as JPEG', async () => {
    const webpBytes = await sharp({
      ...
      .webp()
      .toBuffer();
    const file = new File([webpBytes], 'pareja.webp', { type: 'image/webp' });
  ...
  it('accepts a Samsung-style HEIC avatar and stores it re-encoded as JPEG', async () => {
    const file = new File([HEIC_FIXTURE], 'pareja.heic', { type: 'image/heic' });
